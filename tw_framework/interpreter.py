...
class Interpreter:
    ...
    def run_text(self, text: str, *, base_dir: str = ".", file_path: str = "<memory>", context: Optional[Dict] = None, css_href: Optional[str] = None, debug: bool = False) -> InterpreterResult:
        program = parse_text(text, base_dir=base_dir, file_path=file_path, debug=debug)
        return self.run_program(program, context=context, css_href=css_href)

    def run_file(self, path: str, context: Optional[Dict] = None, css_href: Optional[str] = None, debug: bool = False) -> InterpreterResult:
        program = parse_file(path, debug=debug)
        return self.run_program(program, context=context, css_href=css_href)
