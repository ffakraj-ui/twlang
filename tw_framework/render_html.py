...
def render_program(program: IRProgram, context: Optional[Dict] = None) -> str:
    ...
    # Apply hydration markers for interactive components
    html_output = wrap_interactive_nodes(html_output, program, context=context)
    return html_output
...
def render_streaming(ir_program: IRProgram, *, page_program=None, context: Optional[Dict] = None, css_href: Optional[str] = None) -> Generator[str, None, None]:
    """Yield HTML chunks for streaming SSR."""
    from .streaming import render_program_streaming
...
def render_program_document(ir_program: IRProgram, *, page_program=None, context: Optional[Dict] = None, css_href: Optional[str] = None, debug: bool = False) -> str:
    ...
    config = merged_context.get("config", {})
    use_streaming = bool(config.get("streaming", False))

    if use_streaming:
        chunks = list(render_program_streaming(ir_program, context=merged_context))
        html_output = "".join(chunks)
        # Apply hydration markers
        html_output = wrap_interactive_nodes(html_output, ir_program, context=merged_context)
        return html_output
